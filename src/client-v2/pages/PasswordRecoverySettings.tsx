...
import { PasswordRecoveryFullConfig, PasswordRecoveryAuditLog } from '../types';

const { Title, Paragraph, Text } = Typography;

export default function PasswordRecoverySettings() {
  const t = useT();
  const app = useApp();
  ...
  const handleTestWorkflow = async (values: { account: string }) => {
    ...
    try {
      if (app?.apiClient) {
        ...
        const data = res?.data?.data || res?.data;
        message.success(data?.message || t('Simulate notification workflow successfully'));
      }
    } catch (err: any) {
  ...
  const auditColumns = [
    ...
    { title: t('Client IP'), dataIndex: 'ip', key: 'ip', width: 130 },
    {
      title: t('Audit Time'),
      dataIndex: 'createdAt',
      key: 'createdAt',
      ...
      <Card
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 600 }}>{t('Enterprise Password Recovery')}</span>
          </div>
        }
        loading={loading}
        bordered={false}
              ...
              children: (
                <div>
                  ...
                  <Form form={form} layout="vertical" onFinish={handleSave}>
                    <Form.Item
                      label={t('Enable password recovery')}
                      name="enabled"
                    ...
                    <Form.Item
                      label={t('Button text on login page')}
                      name="buttonText"
                    ...
                    <Space size={24} wrap>
                      ...
                      <Form.Item
                        label={t('Verification code expiration (minutes)')}
                      ...
                      <Form.Item
                        label={t('Daily limit per account')}
                      ...
                      >
                        <Select style={{ width: 220 }}>
                          <Select.Option value="standard">{t('Standard (Letters and Numbers)')}</Select.Option>
                          <Select.Option value="simple">{t('Simple (Length >= 6)')}</Select.Option>
                        </Select>
                      </Form.Item>
                    </Space>
