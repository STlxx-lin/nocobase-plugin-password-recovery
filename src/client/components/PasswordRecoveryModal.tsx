...
export const PasswordRecoveryModal: React.FC<PasswordRecoveryModalProps> = ({
  ...
  const getPasswordStrength = (pwd: string) => {
    ...
    if (score <= 1) return { text: '弱', color: '#ff4d4f', percent: 33 };
    if (score <= 3) return { text: '中', color: '#1677ff', percent: 66 };
  ...
  const sendRequest = async (action: string, payload: any) => {
    const resp = await fetch(`/api/passwordRecovery:${action}`, {
      method: 'POST',
    ...
    if (!resp.ok) {
      throw new Error(data?.message || data?.errors?.[0]?.message || '请求失败，请稍后重试');
    }
  ...
  const handleSendCode = async (values: { account: string }) => {
    ...
    try {
      const resData = await sendRequest('checkAccountAndSendCode', { account: targetAccount });
      ...
      setErrorMessage(err?.message || '账号核验失败，请重试');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendCode = async () => {
    ...
    try {
      const resData = await sendRequest('checkAccountAndSendCode', { account });
      if (resData?.success) {
        setToken(resData.token);
        setCountdown(60);
      } else {
  ...
  const handleResetPassword = async (values: any) => {
    ...
    try {
      ...
      const resData = await sendRequest('resetPassword', payload);
      ...
      setErrorMessage(err?.message || '重置密码失败，请重试');
    } finally {
      setLoading(false);
    }
  };
      ...
      {currentStep === 0 && (
        ...
        >
          ...
          <Paragraph type="secondary" style={{ fontSize: 12, marginBottom: 24, lineHeight: 1.6 }}>
            提示：系统核验账号通过后，将自动触发企业工作流向您的专属通知渠道（企业微信/钉钉/邮件/短信）发送动态验证码。
          </Paragraph>
